...
export const fetchRandomData = (ignoredDays) => {
	let ignoredDaysNumbers = ignoredDays.map(day => {
		switch (day) {
			case "sunday": 
				return 0;
			case "monday": 
...
export const makeRandomDate = (ignoreDays) => {
	...
	let [monthStr, dayStr] = [month, day].map(num => num.toString().length !== 2 ? "0" + num : num.toString());
...
export const makeFinishedGrid = (grid) => {
	...
	grid.forEach((char, idx) => {
		inner.push(char === "." ? false : char.toUpperCase());
		if ((idx + 1) % 15 === 0) {
			outer.push(inner.slice());
			inner = [];
		}
	});
	return outer;
...
export const makeClues = (clues, gridNums) => {
	...
	["across", "down"].forEach(dir => {
		clues[dir].forEach(clue => {
			let [clueNum, clueText] = splitKeepRemainder(clue, ". ", 1);
			let fullClueNum = clueNum + (dir === "across" ? "A" : "D");

			let rawStartCell = gridNums.indexOf(parseInt(clueNum.replace(/\D+/g, '')));
			let startCell = [Math.floor(rawStartCell / 15), rawStartCell % 15];

			cluesMap[fullClueNum] = {
				"startCell": startCell,
				"clue": clueText.replace("&amp;", "&")
			};
		});
	});
